...
public class SwiftSpinner: UIView {
    ...
    public override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? { return self }
    ...
    public class func show(duration: Double,
        ...
        spinner.delay(duration) { SwiftSpinner.hide { completion?() } }
        return spinner
    }
    ...
    public class func hide(_ completion: (() -> Void)? = nil) {
        ...
        if hideCancelsScheduledSpinners { delayedTokens.removeAll() }
    ...
    public override var frame: CGRect {
        didSet { if frame == CGRect.zero { return }
    ...
    private let defaultTitleFont = UIFont(name: "HelveticaNeue", size: 22.0) ?? .systemFont(ofSize: 22.0)
    ...
    required public init?(coder aDecoder: NSCoder) { fatalError("Not coder compliant") }
    ...
    private func spinOuter() {
        ...
        UIView.animate(withDuration: duration, delay: 0.0,
            ...
            self.delay(waitDuration, completion: {
                if self.animating { self.spinOuter() }
            })
        })
    }
    ...
    private func spinInner() {
        ...
        UIView.animate(withDuration: 0.5,
            ...
            self.delay(0.5, completion: {
                if self.animating { self.spinInner() }
            })
        })
    }

    @objc func orientationChangedAction() {
        if SwiftSpinner.containerView() != nil { SwiftSpinner.shared.setNeedsLayout() }
    }
    ...
    func delay(_ seconds: Double, completion: @escaping () -> Void) {
        ...
        DispatchQueue.main.asyncAfter(deadline: popTime) { completion() }
    }
